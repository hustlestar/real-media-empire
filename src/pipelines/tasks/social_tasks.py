...
class SocialTasks:
    def __init__(self, youtube_api_key_path, youtube_channel_name, youtube_channel_id, youtube_tags, youtube_category):
        self.youtube_tags = youtube_tags
        self.youtube_category = youtube_category
        self.youtube_uploader = YouTubeUploader(
            client_secrets_file=youtube_api_key_path, channel_name=str(youtube_channel_name).lower().replace(" ", "_"), channel_id=youtube_channel_id
        )

    def upload_video_to_youtube(
        self, file_path, title="Test Title", description="Test Description", privacy_status=VALID_PRIVACY_STATUSES[1], tags=None
    ):
