...
def prepare_shorts_snippets_video_with_transcript(video_url):
    video_meta, audio_path, video_path = download_video_and_audio(video_url)
    ...
    key_sentences = extract_key_sentences(initial_transcript, num_sentences=round(video_meta.length / 60 / 5))
    ...
    for i, sentence in enumerate(key_sentences):
        ...
        for match in keyword_matches:
            logger.info(f"Timestamp Range: {match['timestamp_range']}")
            logger.info(f"Keyword Text: {match['keyword_text']}\n")
        if keyword_matches:
    ...
    text = preprocess_transcript(initial_transcript_lines)
...
if __name__ == '__main__':
    prepare_shorts_snippets_video_with_transcript("https://www.youtube.com/watch?v=z-mJEZbHFLs")
