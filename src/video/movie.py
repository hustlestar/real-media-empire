...
BG_CLIP_STRATEGIES = {BG_CLIP_CHANGE_EVERY_N_SECONDS, BG_CLIP_CHANGE_PER_SENTENCE, BG_CLIP_CHANGE_PER_LINE}
...
def prepare_video_sublcip(clip: VideoFileClip):
    ...
    clip = concatenate_videoclips([clip.subclip(0, clip.duration - 2), clip.subclip(clip.duration - 2).fx(vfx.fadeout, 2)])
...
def video_with_text(
    ...
    for i, line_and_audio in enumerate(line_to_voice_list):
        ...
        shadow = build_txt_clip(
            text_line, bg_width, bg_width_half, duration, font_size, (pos_1[0] + 0.005, pos_1[1] + 0.005), previous_end, font, color=shadow_color
        )
...
def video_with_quote_and_label(
    ...
    for i, text_line in enumerate(text_lines, start=1):
        ...
        txt_clip = build_txt_clip(
            text_line, bg_video.w, bg_video.w / 2, duration, font_size, (position_tuple[1], position_tuple[0]), 0, font, color=text_color
        )
        txt_clip = txt_clip.crossfadeout(additional_pause / 2)

        shadow_clip = build_txt_clip(
            text_line,
            bg_video.w,
            bg_video.w / 2,
            duration,
            font_size,
            (position_tuple[1] + 0.005, position_tuple[0] + 0.005),
            0,
            font,
            color=shadow_color,
        )
    ...
    if author_label:
        author_clip = build_txt_clip(
            author_label.author, bg_video.w, bg_video.w / 2, duration, font_size / 2, (0.5, 0.9), 0, font, color=text_color
        ).set_opacity(0.8)
        if author_label.title:
            label_clip = build_txt_clip(
                author_label.title, bg_video.w, bg_video.w / 2, duration, font_size / 2, (0.5, 0.95), 0, font, color=text_color
            ).set_opacity(0.8)
            text_clips.append(label_clip)
        text_clips.append(author_clip)

    if extra_label:
        extra_label_clip = build_txt_clip(
            extra_label.text, bg_video.w, bg_video.w / 2, duration, font_size / 2, (0.15, 0.15), 0, font, color=extra_label.color
        )
...
def video_with_text_full_sentence_many_clips(
    ...
    for i, line_and_audio in enumerate(line_to_voice_list):
        ...
        for j, text_line in enumerate(text_lines):
            ...
            if bg_clip_strategy == BG_CLIP_CHANGE_PER_LINE:
                video = (
                    find_matching_video(channel, single_duration)
                    if not is_download_new_video
                    else download_matching_video(thematic_download_generator, single_duration)
                )
            ...
            if bg_clip_strategy == BG_CLIP_CHANGE_PER_SENTENCE and j == len(text_lines) - 1:
                ...
                video = (
                    find_matching_video(channel, required_duration)
                    if not is_download_new_video
                    else download_matching_video(thematic_download_generator, single_duration)
                )
                ...
                logger.info(
                    Fore.GREEN + f"{bg_clip_strategy}: Adding video to bg_clips starting at {required_duration}, with duration of {required_duration}"
                )
                bg_clips.append(video)
    ...
    if bg_clip_strategy == BG_CLIP_CHANGE_EVERY_N_SECONDS:
        while previous_end < final_duration:
            video = (
                find_matching_video(channel, single_clip_duration)
                if not is_download_new_video
                else download_matching_video(thematic_download_generator, single_duration)
            )
            video = video.set_start(previous_end)
            ...
            logger.info(
                Fore.BLUE + f"{bg_clip_strategy}: Adding video to bg_clips starting at {previous_end}, with duration of {single_clip_duration}"
            )
...
def create_thematic_download_generator(channel, is_download_new_video, video_background_themes):
    if is_download_new_video:
        ...
        task = PexelsDownloadTask(
            query=theme_for_the_background_videos,
...
def save_final_video_file(final_clip, result_file):
    final_clip.write_videofile(
        result_file, codec="libx264", audio_codec="aac", temp_audiofile=f"{random.randint(0, 100)}_temp-audio.m4a", remove_temp=True, threads=6
    )
...
def music_video(bg_video, music_file_list: List[str], result_file="music_on_black_bg.mp4", desired_duration=60 * 60, fade_duration=5):
