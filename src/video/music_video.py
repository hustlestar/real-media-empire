...
if __name__ == "__main__":
    bg_video = read_n_video_clips(os.path.join(CONFIG.get("MEDIA_GALLERY_DIR"), "VIDEO", "portrait", f"1080_1920"), 1)[0]

    clips = [read_audio_clip(a) for a in ["1.mp3", "2.mp3", "3.mp3", "4.mp3"]]

    fade_duration = 1

    # Create a composite audio clip from the list of clips with fade-in and fade-out
    all_audio_clips = [
        clip.set_start(sum([c.duration for c in clips[:i]])).audio_fadein(fade_duration).audio_fadeout(fade_duration) for i, clip in enumerate(clips)
    ]
