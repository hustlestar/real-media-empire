...
headers = {"Accept": "application/json", "Content-Type": "application/json", "Authorization": CYBER_VOICE_API_KEY}


def synthesize_text(text, output_file=None, voice_language_code=2, voice_name=37, gender="male"):  # 'female'
    if len(text) > 1000:
        raise Exception("This API takes 1000 symbols max")
    body = {"voice_id": voice_name, "text": text, "format": "mp3"}
...
def sample_all_voices(text):
    for v in list_voices():
        if v["id_lang"] == 2:  # and v['sex'] == 'male':  # english
            ...
            synthesize_text(text, output_file=f"{v['voice_id']}_sample.mp3", voice_name=v["voice_id"])
