...
def upgrade() -> None:
    ...
    op.add_column('content_items',
        sa.Column('detected_language',
                  postgresql.ENUM('en', 'ru', 'es', name='language_enum', create_type=False),
                  nullable=True,
                  comment='Detected language of content'))

    # Add extracted_text_paths JSONB column (for multiple language versions)
    op.add_column('content_items',
    ...
    op.create_table(
        'tags',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False, comment='Unique tag ID'),
        sa.Column('name', sa.String(length=100), nullable=False, comment='Tag name'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False, comment='Tag creation timestamp'),
    ...
    op.create_table(
        ...
        sa.Column('tag_id', postgresql.UUID(as_uuid=True), nullable=False, comment='Reference to tag'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False, comment='Tag assignment timestamp'),
