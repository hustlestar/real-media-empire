...
def upgrade() -> None:
    ...
    # Create content_items table with dialect-appropriate types
    op.create_table(
        'content_items',
        sa.Column('id', sa.String() if not is_postgresql else postgresql.UUID(as_uuid=True), nullable=False, comment='Unique content ID'),
        sa.Column('content_hash', sa.String(length=64), nullable=False, comment='SHA256 hash of content for deduplication'),
        sa.Column('source_type', sa.String(50) if not is_postgresql else postgresql.ENUM('pdf_url', 'pdf_file', 'youtube', 'web', name='source_type_enum', create_type=False), nullable=False, comment='Type of content source'),
        ...
        sa.Column('metadata', sa.JSON() if not is_postgresql else postgresql.JSONB(astext_type=sa.Text()), server_default='{}', nullable=False, comment='Content metadata'),
        sa.Column('user_id', sa.BigInteger(), nullable=False, comment='User who created this content'),
        sa.Column('processing_status', sa.String(50) if not is_postgresql else postgresql.ENUM('pending', 'extracting', 'completed', 'failed', name='processing_status_enum', create_type=False), server_default='pending', nullable=False, comment='Content extraction status'),
        sa.Column('error_message', sa.Text(), nullable=True, comment='Error message if extraction failed'),
        sa.Column('created_at', sa.DateTime(), nullable=False, comment='Content creation timestamp'),
    ...
    # Create processing_jobs table with dialect-appropriate types
    op.create_table(
        'processing_jobs',
        sa.Column('id', sa.String() if not is_postgresql else postgresql.UUID(as_uuid=True), nullable=False, comment='Unique job ID'),
        ...
        sa.Column('processing_type', sa.String(50) if not is_postgresql else postgresql.ENUM('summary', 'mvp_plan', 'content_ideas', name='job_processing_type_enum', create_type=False), nullable=False, comment='Type of AI processing'),
        sa.Column('status', sa.String(50) if not is_postgresql else postgresql.ENUM('pending', 'processing', 'completed', 'failed', name='job_status_enum', create_type=False), server_default='pending', nullable=False, comment='Job execution status'),
