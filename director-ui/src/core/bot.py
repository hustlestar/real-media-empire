...
from core.ai_provider import OpenRouterProvider, MockAIProvider
...
from handlers.message import MessageHandler as MessageHandlerClass
# from support.bot import SupportBot  # Support bot not implemented yet
...
class TelegramBot:
    ...
    def __init__(self, config: BotConfig):
        ...
        self.support_bot = None  # Support bot not implemented yet
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            # Initialize shared services (same as API uses)
            from services.file_storage import FileStorageService
            from services.content_service import ContentService

            self.file_storage = FileStorageService(self.config)
            self.content_service = ContentService(self.database, self.file_storage)
            logger.info("Content service initialized (shared with API)")

            # Determine locales directory path (parent of src if running from src/)
            import os
            current_dir = os.getcwd()
            if current_dir.endswith('/src') or current_dir.endswith('\\src'):
                locales_path = os.path.join(os.path.dirname(current_dir), 'locales')
            else:
                locales_path = os.path.join(current_dir, 'locales')

            self.locale_manager = LocaleManager(locales_dir=locales_path, default_language=self.config.default_language)
            ...
            # Support bot not implemented yet
            ...
            # Pass AI processor, content storage, and message handler to basic handlers
            self.basic_handlers = BasicHandlers(
                locale_manager=self.locale_manager,
                keyboard_manager=self.keyboard_manager,
                database=self.database,
                config=self.config,
                ai_processor=self.message_handler.ai_processor,
                content_storage=self.message_handler.content_storage,
    ...
    async def shutdown_gracefully(self) -> None:
        ...
        # await self._send_shutdown_notification() # Support bot not implemented
